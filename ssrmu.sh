...
#定义变量filepath值为当前运行脚本的目录
#$(dirname "$0")得到当前输入参数的父级路径
#pwd:得到当前执行命令＋参数的父级路径组合的脚本的绝对路径
...
#其中41的位置代表底色, 37的位置是代表字的颜色,最尾巴的\033[0m 关闭所有属性 ,\033[1m 设置高亮度，\033[4m 下划线 ...
...
Download_SSR(){
	...
	#rm -rf manyuser.zip
	git clone https://github.com/328333982/manyuser.git
	mv "/usr/local/manyuser/" "/usr/local/shadowsocksr/"
	cd "shadowsocksr"
